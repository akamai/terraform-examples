...
* - You need to set the JSONNET_PATH environment variable to point to the `jsonnet` directory here (e.g. JSON_PATH=jsonnet/). This allows the Jsonnet application to find the papi schema file which is referenced in each file found in the rules folder. A version of the v2025-01-13 ruleFormat is included in this directory, but Jsonnet rules can use any ruleFormat you wish.
*
