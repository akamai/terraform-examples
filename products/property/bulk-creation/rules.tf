  ...
  rules_v2026_02_16 {
    ...
    behavior {
      origin {
        ...
        hostname                      = each.value.origin_hostname
        http_port                     = 80
        https_port                    = 443
        ip_version                    = "IPV4"
        origin_certificate            = ""
    ...
    children = [
      data.akamai_property_rules_builder.my_property_rule_augment_insights[each.key].json,
...
data "akamai_property_rules_builder" "my_property_rule_augment_insights" {
  ...
  rules_v2026_02_16 {
    ...
    children = [
      data.akamai_property_rules_builder.my_property_rule_traffic_reporting[each.key].json,
...
data "akamai_property_rules_builder" "my_property_rule_traffic_reporting" {
  ...
  rules_v2026_02_16 {
    ...
    behavior {
      cp_code {
        value {
          id = parseint(replace(akamai_cp_code.cp_code[each.key].id, "cpc_", ""), 10)
        }
      }
    }
  }
